...
export default function HomePage() {
  ...
  const balanceAvailable = Number(profile?.balance_available ?? balance);
  const balanceLocked = Number(profile?.balance_locked ?? 0);
  const currentUnits = Number(profile?.units ?? profile?.direct_referrals ?? 0);
  const currentProduction = Number(profile?.production ?? profile?.network_volume ?? 0);
  const nextLevel = levels.find((l) => l.active !== false && Number(l.ordine) > Number(levelInfo?.ordine ?? 0));
  const missingUnits =
    nextLevel?.unita_richieste != null ? Math.max(0, Number(nextLevel.unita_richieste) - currentUnits) : null;
  const missingProduction =
    nextLevel?.produzione_richiesta != null
      ? Math.max(0, Number(nextLevel.produzione_richiesta) - currentProduction)
      : null;
    ...
    return (
      <div className="space-y-6 p-4">
        <div className="usdt-card-gold relative overflow-hidden p-8 text-center">
          ...
          <p className="mt-2 whitespace-pre-line text-sm text-muted-foreground">{t("home.tagline")}</p>
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      >
        ...
        <div className="relative flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">≈ ${balance.toLocaleString()} USD</p>
          </div>
          <UsdtMonogram size={56} letter="U" />
        </div>

        <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
          <div className="rounded-lg border border-primary/20 bg-background/40 p-2">
            <p className="text-[0.65rem] uppercase tracking-wider text-muted-foreground">Disponibile</p>
            <p className="font-display text-base font-bold text-foreground">{balanceAvailable.toFixed(2)} USDT</p>
          </div>
          <div className="rounded-lg border border-primary/20 bg-background/40 p-2">
            <p className="text-[0.65rem] uppercase tracking-wider text-muted-foreground">Bloccato</p>
            <p className="font-display text-base font-bold text-foreground">{balanceLocked.toFixed(2)} USDT</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-center">
          ...
          <div>
            ...
            <p className="font-display mt-0.5 text-base font-bold usdt-gold-text">{getLevelLabel(level)}</p>
          </div>
        </div>
      </motion.section>
      ...
      {/* Livello e prossimi vantaggi */}
      <section className="usdt-card p-4">
        <div className="mb-3 flex items-center justify-between gap-2">
          <h2 className="font-display text-base font-bold">Livello e prossimi vantaggi</h2>
          <button onClick={() => navigate("/qualifiche")} className="flex items-center gap-0.5 text-xs text-primary">
            Qualifiche <ChevronRight className="h-3 w-3" />
          </button>
        </div>
        <div className="grid gap-2 sm:grid-cols-2">
          <RowItem
            ...
            value={nextLevel?.name ?? "Massimo livello"}
            sub={
              nextLevel
                ? `Mancano ${missingUnits ?? 0} unità e ${missingProduction?.toLocaleString() ?? 0} USDT produzione`
                : "Hai raggiunto il livello massimo configurato"
            }
          />
          <RowItem
            icon={<Gift className="h-4 w-4" />}
            label="Bonus potenziale"
            value={nextLevel ? `${Number(nextLevel.bonus_valore ?? 0).toLocaleString()} USDT` : "—"}
            sub="Soggetto a requisiti di livello, rete attiva e produzione"
          />
          <RowItem
            icon={<Lock className="h-4 w-4" />}
            label="Capitale bloccato"
            value={`${balanceLocked.toFixed(2)} USDT`}
            sub="I livelli non sbloccano capitale già investito"
          />
          <RowItem
            icon={<Wallet className="h-4 w-4" />}
            label="Saldo utilizzabile"
            value={`${balanceAvailable.toFixed(2)} USDT`}
            sub="Solo questo saldo è usabile per prelievi o nuove pool"
          />
        </div>
      </section>
      ...
      <section className="usdt-card p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button onClick={() => navigate("/income")} className="flex items-center gap-0.5 text-xs text-primary">
      ...
      <section className="grid grid-cols-2 gap-3">
        <Button className="usdt-btn-gold h-12 gap-2" onClick={() => navigate("/deposit")}>
          <ArrowDownToLine className="h-4 w-4" /> {t("home.deposit")}
        </Button>
        <Button className="usdt-btn-ghost h-12 gap-2" onClick={() => navigate("/withdraw")}>
      ...
      <section className="usdt-card p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button onClick={() => navigate("/fund")} className="flex items-center gap-0.5 text-xs text-primary">
