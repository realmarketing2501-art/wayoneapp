...
// Nessun fallback statico per i piani investimento: se non ci sono piani attivi mostriamo un empty-state.

type Item = { title: string; desc: string };
type LandingLevelCard = {
  id: string;
  name: string;
  min: number;
  max: number;
...
const defaultLevelCards: LandingLevelCard[] = [
  {
    id: "gamma",
    name: "Starter",
    min: 50,
...
export default function Index() {
  ...
  const landingLevels: LandingLevelCard[] = dbLevels.map((l) => ({
    id: String(l.id),
    name: l.name,
    min: Number(l.investimento_min ?? 0),
    max: Number(l.investimento_max ?? 0),
  ...
  const levelsForLanding = landingLevels.length ? landingLevels : defaultLevelCards;
  ...
  return (
    <div className="min-h-screen usdt-bg">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 pb-12 pt-12 sm:pt-20 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="mx-auto mt-4 max-w-lg text-base sm:text-lg text-muted-foreground">{t("landing.hero.sub")}</p>
          </motion.div>
          ...
          >
            {[
              ...
              { value: String(levelsForLanding.length), label: "Livelli" },
            ...
            {flow.map((step, i) => {
              ...
              return (
                <div key={step.title} className="flex items-center gap-3 lg:flex-1 lg:flex-col lg:gap-2">
                  ...
                  {i < flow.length - 1 && <ArrowRight className="h-5 w-5 shrink-0 text-primary/60 lg:rotate-0" />}
                </div>
              );
            })}
          </div>
      ...
      <section className="py-12 sm:py-20">
        <div className="mx-auto max-w-6xl px-4">
          <div className="text-center">
            ...
            <p className="mx-auto mt-3 max-w-xl text-sm text-muted-foreground sm:text-base">{t("landing.plans.sub")}</p>
          </div>
          ...
          {hasPlans ? (
            <>
                ...
                {plans.map((p, i) => (
                  ...
                  >
                    ...
                    <div className="mt-4 space-y-1 text-xs text-muted-foreground">
                      <p>
                        {t("landing.plans.min")}: <span className="text-foreground">{p.min} USDT</span>
                      </p>
                      <p>
                        {t("landing.plans.max")}:{" "}
                        <span className="text-foreground">
                          {p.max === "__UNLIMITED__" ? t("landing.plans.unlimited") : `${p.max} USDT`}
                        </span>
                      </p>
                    </div>
                  </motion.div>
              ...
              <p className="mt-6 text-center text-[0.7rem] text-muted-foreground">
                Target simulativi: non sono promesse garantite. Il capitale investito resta bloccato fino alla scadenza
                del piano.
              </p>
            </>
          ) : (
      ...
      {/* Come funzionano i livelli WAYONE */}
      <div className="usdt-divider mx-auto max-w-6xl" />
      <section className="py-12 sm:py-20">
        <div className="mx-auto max-w-6xl px-4">
          <div className="text-center">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/5 px-3 py-1 text-xs text-primary">
              <Layers className="h-3.5 w-3.5" /> Struttura livelli
            </div>
            <h2 className="font-display text-3xl font-bold sm:text-4xl">
              Come funzionano i <span className="usdt-gold-text">livelli WAYONE</span>
            </h2>
            <p className="mx-auto mt-3 max-w-2xl text-sm text-muted-foreground sm:text-base">
              Investi nei piani, cresci con i livelli. I livelli non sbloccano capitale già bloccato: sbloccano bonus
              potenziali, pool speciali, profondità rete e vantaggi futuri.
            </p>
          </div>

          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {levelsForLanding.map((l, i) => (
              <motion.div
                key={l.id}
                initial={{ opacity: 0, y: 15 }}
                whileInView={{ opacity: 1, y: 0 }}
                ...
                className="usdt-card p-5"
              >
                <div className="mb-3 flex items-center justify-between gap-2">
                  <h3 className="font-display text-xl font-bold usdt-gold-text">{l.name}</h3>
                  <Badge className="text-[0.65rem]">L{i + 1}</Badge>
                </div>
                <div className="space-y-2 text-xs text-muted-foreground">
                  <p>
                    <span className="text-foreground">Investimento:</span> {l.min.toLocaleString("it-IT")}–
                    {l.max.toLocaleString("it-IT")} USDT
                  </p>
                  <p>
                    <span className="text-foreground">75/90 giorni:</span> {l.daily75}% / {l.daily90}% target
                    giornaliero
                  </p>
                  <p>
                    <span className="text-foreground">Rete:</span> {l.units.toLocaleString("it-IT")} unità ·{" "}
                    {l.production.toLocaleString("it-IT")} USDT produzione
                  </p>
                  <p>
                    <span className="text-foreground">Bonus potenziale:</span> {l.bonusPct}% ·{" "}
                    {l.bonusValue.toLocaleString("it-IT")} USDT
                  </p>
                  <p>
                    <span className="text-foreground">Pool sbloccata:</span> Pool {l.name}
                  </p>
                </div>
                <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 p-3 text-[0.7rem] text-muted-foreground">
                  <Lock className="mb-1 h-4 w-4 text-primary" />
                  Il capitale già investito resta bloccato. Questo livello vale per vantaggi futuri.
                </div>
              </motion.div>
            ))}
          </div>
      ...
      {/* Rete, bonus e pool aggiornati */}
      <div className="usdt-divider mx-auto max-w-6xl" />
      <section className="py-12 sm:py-20">
        <div className="mx-auto max-w-6xl px-4">
          <div className="text-center">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/5 px-3 py-1 text-xs text-primary">
              <Network className="h-3.5 w-3.5" /> Rete, bonus e pool
            </div>
            <h2 className="font-display text-3xl font-bold sm:text-4xl">
              Bonus e pool <span className="usdt-gold-text">coerenti con i livelli</span>
            </h2>
            <p className="mx-auto mt-3 max-w-2xl text-sm text-muted-foreground sm:text-base">
              La nuova struttura non usa più bonus generici scollegati. Ogni vantaggio nasce dal livello raggiunto:
              profondità rete, bonus produzione potenziale e pool speciale sbloccata.
            </p>
          </div>

          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {levelsForLanding.slice(1).map((l, i) => (
              <motion.div
                key={`bonus-${l.id}`}
                initial={{ opacity: 0, y: 15 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge className="text-[0.65rem]">Pool {l.name}</Badge>
                </div>
                <div className="mt-4 space-y-2 text-xs text-muted-foreground">
                  <p>
                    <span className="text-foreground">Profondità rete:</span> fino a L{i + 2}
                  </p>
                  <p>
                    <span className="text-foreground">Unità richieste:</span> {l.units.toLocaleString("it-IT")}
                  </p>
                  <p>
                    <span className="text-foreground">Produzione riferimento:</span>{" "}
                    {l.production.toLocaleString("it-IT")} USDT
                  </p>
                  <p>
                    <span className="text-foreground">Bonus potenziale:</span> {l.bonusPct}% ·{" "}
                    {l.bonusValue.toLocaleString("it-IT")} USDT
                  ...
                  <p>
                    <span className="text-foreground">Accesso:</span> Pool {l.name} e vantaggi futuri
                  </p>
                </div>
                <div className="mt-4 rounded-lg border border-amber-400/30 bg-amber-400/5 p-3 text-[0.7rem] text-muted-foreground">
                  Bonus e pool richiedono requisiti attivi. Non modificano gli investimenti già aperti.
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-6xl px-4">
          <div className="grid gap-5 lg:grid-cols-3">
            {[
              {
                icon: Lock,
                title: "Capitale bloccato",
                desc: "Quando apri un piano, il capitale resta bloccato fino alla scadenza. Il livello raggiunto non lo sblocca in anticipo.",
              },
              {
                icon: Layers,
                title: "Livelli = vantaggi futuri",
                desc: "I livelli sbloccano bonus potenziali, profondità rete e pool speciali. Non cambiano retroattivamente i piani già attivi.",
              },
              {
                icon: Wallet,
                title: "Pool con saldo disponibile",
                desc: "Per entrare in una nuova pool puoi usare solo saldo disponibile, bonus disponibili, profitti disponibili o nuovo deposito.",
              },
            ].map((item, i) => {
      ...
      <section className="py-12 sm:py-20">
        <div className="mx-auto max-w-6xl px-4">
          <div className="text-center">
            ...
            <p className="mx-auto mt-3 max-w-xl text-sm text-muted-foreground sm:text-base">
              WAYONE opera con licenza Financial Dealer rilasciata dalle autorità di Saint Vincent and the Grenadines.
              Verifica i nostri documenti ufficiali.
            </p>
          </div>
      ...
      <section className="relative overflow-hidden py-12 sm:py-20">
        ...
        <div className="relative mx-auto max-w-3xl px-4 text-center">
          ...
          <p className="mt-3 text-muted-foreground">{t("landing.cta.sub")}</p>
...
function PhoneLanding() {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
      ...
      <p className="text-[0.55rem] text-muted-foreground whitespace-pre-line">{t("landing.phone.tagline")}</p>
...
function PhoneInvest({ plans }: { plans: PhonePlan[] }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-center font-display text-[0.6rem] font-bold text-foreground">
        {t("landing.phone.choose_plan")}
      </p>
      {phonePlans.map((p, i) => (
        <div
          key={p.name}
        ...
        >
          ...
          <div className="text-right text-[0.5rem]">
            ...
            <p className="font-bold text-primary">
              {p.daily} {t("landing.phone.per_day")}
            </p>
          </div>
        </div>
      ))}
