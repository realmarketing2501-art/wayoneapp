...
export default function CompensationTab() {
  ...
  const { data: specialFunds = [] } = useQuery({
    queryKey: ["admin_special_funds_for_levels"],
    queryFn: async () => {
  ...
  return (
    <Card>
      <CardContent className="p-3.5 sm:p-4 space-y-3">
        ...
        <div className="rounded-lg border border-amber-400/30 bg-amber-500/10 p-2.5 text-[0.7rem] text-muted-foreground">
          I livelli governano bonus potenziali, profondità rete e pool sbloccate. Non sbloccano capitale già investito e
          non modificano retroattivamente i piani attivi.
        </div>
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[0.6rem] text-muted-foreground mt-0.5">
                      codice: {l.id} {l.active === false ? "· disattivato" : ""}
                    </p>
                  </div>
                  {dirty && (
                    <Button
                      size="sm"
                      className="h-7 text-xs gap-1"
                      onClick={() => saveMutation.mutate({ id: l.id, patch: edits[l.id] })}
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                  <div>
                    ...
                    <Input
                      type="number"
                      value={l.ordine ?? ""}
                      onChange={(e) => update(l.id, { ordine: parseInt(e.target.value) || 0 })}
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      value={l.durata_giorni ?? ""}
                      onChange={(e) =>
                        update(l.id, { durata_giorni: e.target.value === "" ? null : parseInt(e.target.value) })
                      }
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      step="0.1"
                      value={l.giornaliero_45 ?? ""}
                      onChange={(e) =>
                        update(l.id, { giornaliero_45: e.target.value === "" ? null : parseFloat(e.target.value) })
                      }
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      step="0.1"
                      value={l.giornaliero_90 ?? ""}
                      onChange={(e) =>
                        update(l.id, { giornaliero_90: e.target.value === "" ? null : parseFloat(e.target.value) })
                      }
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      value={l.settimanale ?? ""}
                      onChange={(e) =>
                        update(l.id, { settimanale: e.target.value === "" ? null : parseFloat(e.target.value) })
                      }
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      value={l.unita_richieste ?? ""}
                      onChange={(e) =>
                        update(l.id, { unita_richieste: e.target.value === "" ? null : parseInt(e.target.value) })
                      }
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      value={l.produzione_richiesta ?? ""}
                      onChange={(e) =>
                        update(l.id, {
                          produzione_richiesta: e.target.value === "" ? null : parseFloat(e.target.value),
                        })
                      }
                      className="h-7 text-xs"
                    />
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      step="0.1"
                      value={l.bonus_percentuale}
                      onChange={(e) => update(l.id, { bonus_percentuale: parseFloat(e.target.value) || 0 })}
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      value={l.bonus_valore}
                      onChange={(e) => update(l.id, { bonus_valore: parseFloat(e.target.value) || 0 })}
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      value={l.investimento_min ?? ""}
                      onChange={(e) =>
                        update(l.id, { investimento_min: e.target.value === "" ? null : parseFloat(e.target.value) })
                      }
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      type="number"
                      value={l.investimento_max ?? ""}
                      onChange={(e) =>
                        update(l.id, { investimento_max: e.target.value === "" ? null : parseFloat(e.target.value) })
                      }
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      value={l.prossimo_livello ?? ""}
                      onChange={(e) => update(l.id, { prossimo_livello: (e.target.value || null) as any })}
                      className="h-7 text-xs"
                    />
                  </div>
                  <div>
                    <Label className="text-[0.6rem] text-muted-foreground">Pool collegata</Label>
                    <select
                      value={l.pool_id ?? ""}
                      onChange={(e) => update(l.id, { pool_id: e.target.value || null })}
                      className="h-7 w-full rounded-md border border-input bg-background px-2 text-xs"
                    >
                      <option value="">Nessuna / da configurare</option>
                      {specialFunds.map((f) => (
                  ...
                  <div className="flex items-end gap-3">
                    <label className="flex items-center gap-1.5 text-[0.65rem] text-muted-foreground">
                      <input
                        type="checkbox"
                        checked={l.rete}
                        onChange={(e) => update(l.id, { rete: e.target.checked })}
                      />
                      Rete attiva
                    </label>
                    <label className="flex items-center gap-1.5 text-[0.65rem] text-muted-foreground">
                  ...
                  <div className="col-span-2 sm:col-span-4">
                    ...
                    <Input
                      value={l.note ?? ""}
                      onChange={(e) => update(l.id, { note: e.target.value || null })}
                      className="h-7 text-xs"
                    />
                  </div>
                </div>
